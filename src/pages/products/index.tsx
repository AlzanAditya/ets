...
import type { ProductInsert, ProductRow, ProductImageRow } from "@/types/database"
import {
  moveTempToDraft,
  moveDraftToProduct,
  cleanupDraft,
...
interface ProductDraft {
  ...
  imagePaths: Array<{ storagePath: string; thumbPath: string; sortOrder: number }>
}
...
function emptyFields(): ProductDraftFields {
  return {
    nomor_seri: '', product_code: '', nama_produk: '', category: '', brand: '',
    tipe_kode: '', tahun_pembuatan: '', input: '', output: '', frekuensi: '',
    jumlah_socket: '', range_daya: '', soft_fuse_protection: '', hard_fuse_protection: '',
    ground_output: '', tambahan_optional: '', current_branch_id: '', current_client_id: '',
    status: 'active',
  }
}
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editTarget) {
        ...
        const deletedDbImages = initialDbImages.filter((img) => !currentDbImageIds.has(img.id))

        for (const img of deletedDbImages) {
          await productsService.deleteProductImage(img.id)
          try {
            await deleteFiles([img.storage_path, img.thumbnail_path].filter(Boolean) as string[])
          } catch (err) {
        ...
        const allFinalImagesWithIds = drawerImages.map((img) => {
          ...
          const matchedDb = newlyInsertedImages.find(
            (dbImg) => dbImg.storage_path.endsWith(fileName)
          )
          return {
            ...img,
        ...
        const validUpdates = allFinalImagesWithIds
          ...
          .filter((x): x is { id: string; sort_order: number } => x.id !== null)
    ...
    try {
      ...
      if (tempImages.length > 0 && !activeDraftId) {
        const pairs = await moveTempToDraft(
          sessionId,
          draftId,
          tempImages.map((img) => ({ fullPath: img.storagePath, thumbPath: img.thumbPath })),
        )
      ...
      const next = [
        draft,
        ...drafts.filter((d) => d.draftId !== draftId),
      ]
      setDrafts(next)
      saveDrafts(next)
  ...
  const FormFields = (
    <div className="flex flex-col gap-4 text-sm">
      ...
      <div className="flex flex-col gap-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Identitas Produk</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="nomor_seri">Nomor Seri *</Label>
          <Input id="nomor_seri" value={fields.nomor_seri} onChange={(e) => setField("nomor_seri", e.target.value)} placeholder="SN-..." disabled={!!editTarget} />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="product_code">Kode Produk</Label>
          <Input id="product_code" value={fields.product_code} onChange={(e) => setField("product_code", e.target.value)} placeholder="PRD-..." />
        </div>
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="nama_produk">Nama Produk *</Label>
        <Input id="nama_produk" value={fields.nama_produk} onChange={(e) => setField("nama_produk", e.target.value)} placeholder="Nama lengkap produk" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="category">Kategori</Label>
          <Input id="category" value={fields.category} onChange={(e) => setField("category", e.target.value)} placeholder="e.g. UPS, Stabilizer" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="brand">Brand</Label>
          <Input id="brand" value={fields.brand} onChange={(e) => setField("brand", e.target.value)} placeholder="e.g. Liebert, APC" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="tipe_kode">Tipe / Kode Model</Label>
          <Input id="tipe_kode" value={fields.tipe_kode} onChange={(e) => setField("tipe_kode", e.target.value)} />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="tahun_pembuatan">Tahun Pembuatan</Label>
          <Input id="tahun_pembuatan" type="number" min="1990" max="2099" value={fields.tahun_pembuatan} onChange={(e) => setField("tahun_pembuatan", e.target.value)} placeholder="2024" />
        </div>
      </div>

      {/* Group: Spesifikasi Elektrikal */}
      <div className="flex flex-col gap-1 pt-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Spesifikasi Elektrikal</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="input">Input</Label>
          <Input id="input" value={fields.input} onChange={(e) => setField("input", e.target.value)} placeholder="220V / 1Ph / 50Hz" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="output">Output</Label>
          <Input id="output" value={fields.output} onChange={(e) => setField("output", e.target.value)} placeholder="220V / 1Ph / 50Hz" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="frekuensi">Frekuensi</Label>
          <Input id="frekuensi" value={fields.frekuensi} onChange={(e) => setField("frekuensi", e.target.value)} placeholder="50 Hz" />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="jumlah_socket">Jumlah Socket</Label>
          <Input id="jumlah_socket" type="number" min="0" value={fields.jumlah_socket} onChange={(e) => setField("jumlah_socket", e.target.value)} />
        </div>
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="range_daya">Range Daya</Label>
        <Input id="range_daya" value={fields.range_daya} onChange={(e) => setField("range_daya", e.target.value)} placeholder="1000VA – 3000VA" />
      </div>

      {/* Group: Proteksi */}
      <div className="flex flex-col gap-1 pt-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Proteksi</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="soft_fuse_protection">Soft Fuse Protection</Label>
          <Input id="soft_fuse_protection" value={fields.soft_fuse_protection} onChange={(e) => setField("soft_fuse_protection", e.target.value)} />
        </div>
        <div className="flex flex-col gap-1.5">
          <Label htmlFor="hard_fuse_protection">Hard Fuse Protection</Label>
          <Input id="hard_fuse_protection" value={fields.hard_fuse_protection} onChange={(e) => setField("hard_fuse_protection", e.target.value)} />
        </div>
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="ground_output">Ground Output</Label>
        <Input id="ground_output" value={fields.ground_output} onChange={(e) => setField("ground_output", e.target.value)} />
      </div>

      {/* Group: Lokasi & Status */}
      <div className="flex flex-col gap-1 pt-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Lokasi & Status</p>
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="status">Status</Label>
        <Select value={fields.status} onValueChange={(v) => setField("status", v as ProductRow['status'])}>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Select
          ...
          onValueChange={(v) => setField("current_branch_id", v === NO_SELECTION_VALUE ? "" : v)}
        >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Select
          ...
          onValueChange={(v) => setField("current_client_id", v === NO_SELECTION_VALUE ? "" : v)}
        >
      ...
      {/* Group: Catatan */}
      <div className="flex flex-col gap-1 pt-1">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Catatan Tambahan</p>
      </div>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="tambahan_optional">Keterangan Opsional</Label>
        <Input id="tambahan_optional" value={fields.tambahan_optional} onChange={(e) => setField("tambahan_optional", e.target.value)} placeholder="Info tambahan..." />
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <TableDrawer
        ...
        title={editTarget ? editTarget.nama_produk : (isDraftMode ? "Edit Draft" : "Tambah Produk Baru")}
